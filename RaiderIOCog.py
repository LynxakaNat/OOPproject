...
class RaiderIOCog(commands.Cog):
    ...
    async def spy(self, ctx, character: str, realm: str, region: str):
        ...
        if data['faction'] == 'horde':
            ...
            adjective = "filthy"
        embedded_mess = discord.Embed(title=tit, description=f"They are a {data['gender']} "
                                                             f"{data['race']} {data['class']}. "
                                                             f"They belong to the {adjective} {data['faction']}",
                                      color=0x394A8C)
        embedded_mess.set_thumbnail(url=data['thumbnail_url'])
    ...
    async def mythicscore(self, ctx, character: str, realm: str, region: str):
        ...
        data2 = self.parser.ParseChar(region, realm, character, field2)
        diff = int(data['mythic_plus_scores_by_season'][0]['segments']['all']['score']) - int(
            data2['mythic_plus_scores_by_season'][0]['segments']['all']['score'])
        adjective = ""
        if diff >= 0:
            adjective = "increase"
        elif diff < 0:
        ...
        embedded_mess = discord.Embed(title=f"This is {data['name']}'s current season's mythic score:",
                                      description=f"{data['mythic_plus_scores_by_season'][0]['segments']['all']['score']}\n"
                                                  f"Compared to the previous season "
                                                  f"it is a {abs(diff)} points {adjective}",
                                      color=readable_hex)
        await ctx.send(embed=embedded_mess)
    ...
    async def mythicranking(self, ctx, character: str, realm: str, region: str, spec: str):
        ...
        embedded_mess = discord.Embed(title=f"This is {data['name']}'s ranking score:", color=0x394A8C)
        embedded_mess.add_field(name='Overall:', value=f"> World: {data['mythic_plus_ranks']['overall']['world']}\n>"
                                                       f" Region: {data['mythic_plus_ranks']['overall']['region']}\n>"
                                                       f" Realm: {data['mythic_plus_ranks']['overall']['realm']}")

        embedded_mess.add_field(name=f"{data['class']} class:",
        ...
        embedded_mess.add_field(name=f" {data['class']} {spec}  spec:",
    ...
    async def lastruns(self, ctx, character: str, realm: str, region: str):
        ...
        data = self.parser.ParseChar(region, realm, character, field)
        embedded_mess = discord.Embed(title="3 most recent mythic+ runs", color=0x394A8C)
    ...
    async def bestruns(self, ctx, character: str, realm: str, region: str):
        ...
        data = self.parser.ParseChar(region, realm, character, field)
        embedded_mess = discord.Embed(title="3 best mythic+ runs", color=0x394A8C)
    ...
    async def affix(self, ctx, region: str, locale: str = None):
        ...
        embedded_mess = discord.Embed(title="This week's affixes are:", color=0x394A8C)
        for affix in range(3):
            embedded_mess.add_field(name=f"{data['affix_details'][affix]['name']}",
    ...
    async def run(self, ctx, run_id: str, season: str = None):
        ...
        if data['time_remaining_ms'] > 0:
            ...
            descr = "no"
        embedded_mess = discord.Embed(title=data['dungeon']['name'], color=0x394A8C)
        embedded_mess.add_field(name="Keystone level:", value=data['mythic_level'], inline=False)
        embedded_mess.add_field(name="Time of completion:", value=time, inline=False)
        ...
        embedded_mess.add_field(name="Members:", value=roster_info, inline=False)
        embedded_mess.add_field(name="Deaths:", value=len(data['logged_details']['deaths']))
        embedded_mess.add_field(name="Keystone timed:", value=descr)

        picture = "https://cdnassets.raider.io" + data['dungeon']['icon_url']
        embedded_mess.set_thumbnail(url=picture)
        await ctx.send(embed=embedded_mess)
